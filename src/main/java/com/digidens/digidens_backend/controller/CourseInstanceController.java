...
public class CourseInstanceController {
	...
	public List<CourseInstanceDTO> getAllCourseInstances() {
		return courseInstanceRepository.findAll().stream().map(CourseInstanceDTO::fromEntity)
	...
	public ResponseEntity<CourseInstanceDTO> getCourseInstanceById(@PathVariable Long id) {
		return courseInstanceRepository.findById(id).map(CourseInstanceDTO::fromEntity).map(ResponseEntity::ok)
				.orElse(ResponseEntity.notFound().build());
	}

	// --- Hae kaikki kurssitoteutukset tietylle kurssille DTO:ina ---
	@GetMapping("/course/{courseId}")
	public List<CourseInstanceDTO> getCourseInstancesByCourse(@PathVariable Long courseId) {
		return courseInstanceRepository.findAllByCourseId(courseId).stream().map(CourseInstanceDTO::fromEntity)
				.collect(Collectors.toList());
	}

	// --- Hae kaikki kurssitoteutukset tietylle opettajalle DTO:ina ---
	@GetMapping("/teacher/{teacherId}")
	public List<CourseInstanceDTO> getCourseInstancesByTeacher(@PathVariable Long teacherId) {
		return courseInstanceRepository.findAllByTeacherId(teacherId).stream().map(CourseInstanceDTO::fromEntity)
				.collect(Collectors.toList());
	}

	// --- Luo uusi kurssitoteutus (palauttaa entityn) ---
	@PostMapping
	...
	// --- Hae opiskelijat ja heidän edistymisensä yhdelle kurssitoteutukselle
	// DTO:ina ---
