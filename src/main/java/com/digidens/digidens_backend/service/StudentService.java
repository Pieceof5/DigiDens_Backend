...
public class StudentService {
	...
	public List<StudentCourseProgressDTO> getCoursesWithProgressByStudentId(Long id) {
		...
		User student = userRepository.findById(id).orElseThrow(() -> new RuntimeException("Student not found: " + id));
		...
		return instances.stream().map(ci -> {
			...
			return new StudentCourseProgressDTO(ci.getId(),
					ci.getCourse() != null ? ci.getCourse().getName() : "(Ei kurssia)", ci.getInstanceCode(),
					ci.getStartDate(), ci.getEndDate(), completedTasks, totalTasks);
	...
	public List<StudentCourseProgressDTO> getStudentsWithProgressByCourseInstanceId(Long courseInstanceId) {
		...
		return instance.getStudents().stream().map(student -> {
			int totalTasks = (int) (Math.random() * 10 + 5);
			int completedTasks = (int) (Math.random() * (totalTasks + 1));

			return new StudentCourseProgressDTO(instance.getId(),
					instance.getCourse() != null ? instance.getCourse().getName() : "(Ei kurssia)",
					instance.getInstanceCode(), instance.getStartDate(), instance.getEndDate(), completedTasks,
					totalTasks);
		}).collect(Collectors.toList());
	}
}